...
const OneLineReviewItem = ({ oneLineReview }) => {
  return (
    <Link to={`/${oneLineReview.movieId}/${oneLineReview.title}${oneLineReview.posterPath.split(".")[0]}`}>
      <div key={oneLineReview.oneLineReviewId} className="flex bg-mGray gap-2 w-full rounded-xl my-6 flex-col md:flex-row p-2">
        <div className="flex text-mYellow items-center md:basis-2/12">
          <MyStars key={oneLineReview.title} star={oneLineReview.oneLineReviewStar} />
        </div>
